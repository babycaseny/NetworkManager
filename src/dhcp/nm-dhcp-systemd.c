                     ...
                     GError **error)
{
	...
	LOG_LEASE (LOGD_DHCP4, "%s '%u'",
	           request_string (dhcp4_requests, DHCP_OPTION_SUBNET_MASK),
	           (guint) a_plen);
	add_option (options,
	            dhcp4_requests,
	...
	add_option_u64 (options, dhcp4_requests, DHCP_OPTION_IP_ADDRESS_LEASE_TIME, a_lifetime);
	...
	if (sd_dhcp_lease_get_server_identifier (lease, &server_id) >= 0) {
		...
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_SERVER_ID),
		           addr_str);
		add_option (options, dhcp4_requests, DHCP_OPTION_SERVER_ID, addr_str);
	}

	if (sd_dhcp_lease_get_broadcast (lease, &broadcast) >= 0) {
		nm_utils_inet4_ntop (broadcast.s_addr, addr_str);
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_BROADCAST),
		           addr_str);
		add_option (options, dhcp4_requests, DHCP_OPTION_BROADCAST, addr_str);
	}
	...
	if (num > 0) {
		...
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_DOMAIN_NAME_SERVER),
		           str->str);
	...
	if (num > 0) {
		...
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_DOMAIN_SEARCH_LIST),
		           str->str);
	...
	if (sd_dhcp_lease_get_domainname (lease, &s) >= 0) {
		...
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_DOMAIN_NAME),
		           s);
		add_option (options, dhcp4_requests, DHCP_OPTION_DOMAIN_NAME, s);
	...
	if (sd_dhcp_lease_get_hostname (lease, &s) >= 0) {
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_HOST_NAME),
		           s);
		add_option (options, dhcp4_requests, DHCP_OPTION_HOST_NAME, s);
	}
	...
	if (num > 0) {
		...
		for (i = 0; i < num; i++) {
			...
			if (!NM_IN_SET (option, DHCP_OPTION_CLASSLESS_STATIC_ROUTE,
			...
			LOG_LEASE (LOGD_DHCP4,
			           "%sstatic_route %s/%d gw %s",
			             option == DHCP_OPTION_CLASSLESS_STATIC_ROUTE
			           ? "rfc3442_classless_"
			           : "",
			           network_net_str,
			           (int) r_plen,
			           gateway_str);
			g_string_append_printf (nm_gstring_add_space_delimiter (  option == DHCP_OPTION_CLASSLESS_STATIC_ROUTE
			                                                        ? str_classless
			                                                        : str_static),
			                        "%s/%d %s",
			                        network_net_str,
			...
			if (   option == DHCP_OPTION_STATIC_ROUTE
			    && has_classless_route) {
			...
			if (   r_plen == 0
			    && option == DHCP_OPTION_STATIC_ROUTE) {
		...
		if (str_classless && str_classless->len > 0)
			add_option (options, dhcp4_requests, DHCP_OPTION_CLASSLESS_STATIC_ROUTE, str_classless->str);
		if (str_static && str_static->len > 0)
			add_option (options, dhcp4_requests, DHCP_OPTION_STATIC_ROUTE, str_static->str);
	}
	...
	if (num > 0) {
		...
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_ROUTER),
		           str->str);
	...
	if (   sd_dhcp_lease_get_mtu (lease, &mtu) >= 0
	    && mtu) {
		LOG_LEASE (LOGD_DHCP4, "%s '%u'",
		           request_string (dhcp4_requests, DHCP_OPTION_INTERFACE_MTU),
		           mtu);
	...
	if (num > 0) {
		...
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_NTP_SERVER),
		           str->str);
	...
	if (sd_dhcp_lease_get_root_path (lease, &s) >= 0) {
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_ROOT_PATH),
		           s);
		add_option (options, dhcp4_requests, DHCP_OPTION_ROOT_PATH, s);
	}

	if (sd_dhcp_lease_get_t1 (lease, &renewal) >= 0) {
		LOG_LEASE (LOGD_DHCP4, "%s '%u'",
		           request_string (dhcp4_requests, DHCP_OPTION_RENEWAL_T1_TIME),
		           renewal);
		add_option_u64 (options, dhcp4_requests, DHCP_OPTION_RENEWAL_T1_TIME, renewal);
	}

	if (sd_dhcp_lease_get_t2 (lease, &rebinding) >= 0) {
		LOG_LEASE (LOGD_DHCP4, "%s '%u'",
		           request_string (dhcp4_requests, DHCP_OPTION_REBINDING_T2_TIME),
		           rebinding);
		add_option_u64 (options, dhcp4_requests, DHCP_OPTION_REBINDING_T2_TIME, rebinding);
	}

	if (sd_dhcp_lease_get_timezone (lease, &s) >= 0) {
		LOG_LEASE (LOGD_DHCP4, "%s '%s'",
		           request_string (dhcp4_requests, DHCP_OPTION_NEW_TZDB_TIMEZONE),
		           s);
		add_option (options, dhcp4_requests, DHCP_OPTION_NEW_TZDB_TIMEZONE, s);
	}
                     ...
                     GError **error)
{
	...
	if (num > 0) {
		...
		LOG_LEASE (LOGD_DHCP6, "%s '%s'",
		           request_string (dhcp6_requests, DHCP6_OPTION_DNS_SERVERS),
		           str->str);
		add_option (options, dhcp6_requests, DHCP6_OPTION_DNS_SERVERS, str->str);
	}
	...
	if (num > 0) {
		...
		LOG_LEASE (LOGD_DHCP6, "%s '%s'",
		           request_string (dhcp6_requests, DHCP6_OPTION_DOMAIN_LIST),
		           str->str);
		add_option (options, dhcp6_requests, DHCP6_OPTION_DOMAIN_LIST, str->str);
	}
